...
export default function DashboardLayout({
  ...
  // Guard against re-subscribing to badge subscriptions
  const subscriptionsActiveRef = useRef<{ uid: string | null; showAdmin: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!uid) {
      ...
      subscriptionsActiveRef.current = null;
      return;
    }

    // Guard: if we're already subscribed for this uid + admin state, don't re-subscribe
    const currentState = { uid, showAdmin: showAdminNav };
    if (subscriptionsActiveRef.current?.uid === uid && 
        subscriptionsActiveRef.current?.showAdmin === showAdminNav) {
      return; // Already subscribed with same state
    }

    // Mark as active
    subscriptionsActiveRef.current = currentState;
    ...
    <RequireAuth>
      ...
      <div className="min-h-screen bg-background">
        <div className="flex min-h-screen w-full">
          ...
          <aside
            className={cn(
              'hidden md:block w-64 shrink-0 border-r border-border/50 bg-card',
