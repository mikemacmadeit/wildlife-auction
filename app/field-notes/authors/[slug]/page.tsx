...
export default async function FieldNotesAuthorPage({ params }: { params: { slug: string } }) {
  const posts = await getFieldNotesByAuthorSlug(params.slug);
  const name = posts[0]?.author || null;

  if (!posts.length || !name) {
    return (
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 md:px-6 py-10 space-y-6">
          <Link href="/field-notes/authors" className="text-sm font-semibold text-primary hover:underline underline-offset-4">
            ← Back to Authors
          </Link>
      ...
      <div className="container mx-auto px-4 md:px-6 py-10 space-y-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="space-y-1">
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight font-founders">{name}</h1>
            <p className="text-sm text-muted-foreground">Field Notes posts by {name}.</p>
          </div>
