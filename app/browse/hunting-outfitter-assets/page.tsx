...
export default function HuntingOutfitterAssetsBrowsePage() {
  ...
  const getBrowseFilters = useCallback((): BrowseFilters => {
    const browseFilters: BrowseFilters = { status: 'active', category: 'hunting_outfitter_assets' };
  ...
  const filteredListings = useMemo(() => {
    ...
    if (debouncedSearchQuery.trim()) {
      ...
      result = result.filter((listing) => {
        ...
        const typeMatch =
          listing.attributes && 'equipmentType' in (listing.attributes as any)
            ? String((listing.attributes as any).equipmentType || '').toLowerCase().includes(q)
            : false;
        const makeMatch =
          listing.attributes && 'make' in (listing.attributes as any)
            ? String((listing.attributes as any).make || '').toLowerCase().includes(q)
            : false;
        const modelMatch =
          listing.attributes && 'model' in (listing.attributes as any)
            ? String((listing.attributes as any).model || '').toLowerCase().includes(q)
            : false;
        return titleMatch || descMatch || typeMatch || makeMatch || modelMatch;
      });
    }
    return result;
      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 icon-primary-color mask-icon-hunting-blind" />
            <h1 className="text-3xl md:text-4xl font-extrabold text-foreground">Hunting &amp; Outfitter Assets</h1>
          </div>
          <p className="text-base md:text-lg text-muted-foreground">
            Blinds, feeders, camera systems, water systems, fencing, and other hunting/outfitter property assets.
          </p>
        </div>
        ...
        {error ? (
          ...
          <>
            {viewMode === 'list' ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={cn('grid gap-6', 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3')}>
                <AnimatePresence>
                  {filteredListings.map((listing) =>
                    listing.featured ? (
