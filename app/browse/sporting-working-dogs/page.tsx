...
export default function SportingWorkingDogsBrowsePage() {
  ...
  const getBrowseFilters = useCallback((): BrowseFilters => {
    const browseFilters: BrowseFilters = { status: 'active', category: 'sporting_working_dogs' };
  ...
  const filteredListings = useMemo(() => {
    ...
    if (debouncedSearchQuery.trim()) {
      ...
      result = result.filter((listing) => {
        ...
        const breedTrainingMatch =
          listing.attributes && 'breed' in (listing.attributes as any)
            ? `${String((listing.attributes as any)?.breed || '')} ${String((listing.attributes as any)?.trainingDescription || '')}`
                .toLowerCase()
                .includes(q)
            : false;
        return titleMatch || descMatch || breedTrainingMatch;
      });
    }
    return result;
      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 icon-primary-color mask-icon-dog" />
            <h1 className="text-3xl md:text-4xl font-extrabold text-foreground">Sporting &amp; Working Dogs</h1>
          </div>
          <p className="text-base md:text-lg text-muted-foreground">
            Browse sporting &amp; working dog listings (Texas-only transactions).
          </p>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  type="search"
                  placeholder="Search by title, description, breed, or training..."
                  value={searchQuery}
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="text-sm text-muted-foreground">
            Showing <span className="font-semibold text-foreground">{filteredListings.length}</span> listing(s)
          </div>
