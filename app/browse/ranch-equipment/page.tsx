...
export default function RanchEquipmentBrowsePage() {
  ...
  const filteredListings = useMemo(() => {
    ...
    if (debouncedSearchQuery.trim()) {
      ...
      result = result.filter((listing) => {
        ...
        const typeMatch = listing.attributes && 'equipmentType' in listing.attributes 
          ? (listing.attributes as any).equipmentType?.toLowerCase().includes(query)
          : false;
        const makeMatch = listing.attributes && 'make' in listing.attributes 
          ? (listing.attributes as any).make?.toLowerCase().includes(query)
          : false;
        const modelMatch = listing.attributes && 'model' in listing.attributes 
          ...
          <Card>
            <CardContent className="pt-12 pb-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {searchQuery ? 'Try adjusting your search query.' : 'No ranch equipment listings available at this time.'}
              </p>
            </CardContent>
          </Card>
        ) : (
