...
export default function HorseEquestrianBrowsePage() {
  ...
  const handleViewModeChange = (mode: ViewMode) => {
    setViewMode(mode);
    if (typeof window !== 'undefined') localStorage.setItem('browse-view-mode', mode);
  };
  ...
  const filteredListings = useMemo(() => {
    ...
    if (debouncedSearchQuery.trim()) {
      ...
      result = result.filter((listing) => {
        ...
        const idMatch =
          listing.attributes && 'identification' in (listing.attributes as any)
            ? String((listing.attributes as any)?.identification?.microchip || '').toLowerCase().includes(q) ||
              String((listing.attributes as any)?.registrationNumber || '').toLowerCase().includes(q)
            : false;
      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            />
            <h1 className="text-3xl md:text-4xl font-extrabold text-foreground">Horse &amp; Equestrian</h1>
          </div>
          <p className="text-base md:text-lg text-muted-foreground">
            Browse horses and equestrian listings (Texas-only transactions; Bill of Sale available in-platform).
          </p>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  type="search"
                  placeholder="Search by title, description, microchip, or registration..."
                  value={searchQuery}
