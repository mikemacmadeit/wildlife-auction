...
/**
 * Skeleton for seller route content. Uses the SAME outer wrapper as real
 * seller pages (min-h-screen, container, padding, max-w-7xl) so content
  ...
  return (
    <div className={cn('min-h-screen bg-background pb-20 md:pb-6', className)}>
      <div className="container mx-auto px-4 py-6 md:py-8 max-w-7xl space-y-6 md:space-y-8">
        {/* Header — same as typical seller page */}
        <div>
          <Skeleton className="h-8 md:h-9 w-48 sm:w-64 rounded-lg mb-2" />
          <Skeleton className="h-4 w-full max-w-md rounded" />
        </div>
        {/* Filter/search card — matches offers, sales, etc. */}
        ...
        {/* List/cards area — same structure as order/card rows */}
